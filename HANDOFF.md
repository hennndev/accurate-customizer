...
- **Penambahan Opsi Mapping Manual Nomor Faktur (Detail Invoice) pada Modal Migrasi:** 
  ...
  - **Implementasi:** 
    ...
    2. `resources/views/migrate/index.blade.php`: Menambahkan state `modalCustomInvoiceMappings` dan tampilan input field interaktif untuk setiap faktur terkait pada tabel preview modal. Membungkus baris transaksi dan baris detail faktur ke dalam elemen `<tbody class="...">` per item agar sesuai dengan aturan *single root element* Alpine.js `x-for`.
  ...
  - **Peningkatan Layout UI & Fix Full Width:** Memperbaiki bug rendering Alpine.js `x-for` pada `resources/views/migrate/index.blade.php`. Sekarang modal (`sm:max-w-5xl lg:max-w-6xl w-full`) dan tabel pratinjau (`table-fixed w-full min-w-full`) tampil 100% melebar penuh dan panel **Mapping Manual Faktur Terkait** dipastikan muncul konsisten di bawah setiap transaksi pelunasan.
- **Bugfix Mapping Manual InvoiceNo Sales-Receipt Diabaikan:** 
  - **Penyebab:** `SalesReceiptHandler::transformDetail()` menggantikan item transaksi hanya dengan daftar `allowedFields`, sehingga kunci internal `_custom_invoice_mappings` (yang membawa nomor `invoiceNo` hasil edit manual di modal) terbuang. `DataCleaner::cleanDataItem()` hanya mengembalikan `id` dan `_custom_number` setelah handler, sehingga edit manual `invoiceNo` pada sales-receipt tidak pernah terkirim ke Accurate. Payload tetap memakai nomor faktur sumber lama → faktur tidak ditemukan di target → transaksi tercatat `success` namun data tidak terbentuk di Accurate.
  - **Solusi:** `DataCleaner.php` kini menyimpan dan mengembalikan `_custom_invoice_mappings` setelah `transformDetail` (paralel dengan `_custom_number`). Memperbaiki juga variabel tidak terdefinisi `$subItem` pada fallback ekstraksi nomor faktur menjadi `$subValue`.
