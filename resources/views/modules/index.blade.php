<x-app-layout>
  ...
  <div class="flex flex-col gap-10"
       x-data="{
           ...
           modalStartDate: '',
           modalEndDate: '',
           modalFilterType: 'range',
           
           openCaptureModal(moduleName, moduleSlug, moduleType = 'transaction') {
               this.modalModuleName = moduleName;
               this.modalModuleSlug = moduleSlug;
               this.modalModuleType = moduleType;
               this.modalCaptureMode = this.captureModes[moduleSlug] || 'list_and_detail';
               this.modalStartDate = document.getElementById('start_date')?.value || '';
               this.modalEndDate = document.getElementById('end_date')?.value || '';
               this.modalFilterType = 'range';
               this.showCaptureModal = true;
           },
           
           closeCaptureModal() {
               this.showCaptureModal = false;
               this.modalModuleName = '';
               this.modalModuleSlug = '';
               this.modalModuleType = '';
               this.modalCaptureMode = '';
           },
           ...
           async executeCapture() {
               const captureMode = this.modalCaptureMode;
               const moduleSlug = this.modalModuleSlug;
               const moduleName = this.modalModuleName;
               ...
               try {
                   const response = await fetch(`/modules/${moduleSlug}/capture`, {
                       ...
                       body: JSON.stringify({
                           ...
                           filter_type: this.modalFilterType,
                           capture_page: 1,
          ...
          <form id="dbSelectForm"
            ...
            <div x-data="{
                ...
                selectDb(dbId, dbAlias, dbData) {
                    ...
                    // Clear filter inputs when switching database
                 ...
                 @db-switching.window="switchingDb = $event.detail.switching"
    ...
    @if ($current_database_name)
      {{-- Date Filter Form --}}
      <div class="w-full bg-white rounded-xl shadow-md border border-gray-200 p-5"
           id="filter-section">
        <form method="GET"
          ...
          <div class="flex flex-col md:flex-row items-stretch md:items-end gap-4">
            ...
            <div class="flex gap-2">
              <button type="submit"
                      class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors flex items-center gap-2 whitespace-nowrap">
                <svg xmlns="http://www.w3.org/2000/svg"
                     fill="none"
                     viewBox="0 0 24 24"
                     stroke-width="1.5"
    ...
    <div class="flex items-center gap-3 flex-wrap">
      <button @click="activeFilter = 'all'"
              :class="activeFilter === 'all' ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'"
      ...
      <button @click="activeFilter = 'master'"
              :class="activeFilter === 'master' ? 'bg-purple-600 text-white shadow-lg' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'"
              class="px-5 py-2.5 rounded-lg font-medium transition-all duration-200 flex items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg"
             fill="none"
             viewBox="0 0 24 24"
        ...
        <span x-show="activeFilter === 'master'"
              class="ml-1 px-2 py-0.5 bg-white/20 rounded-full text-xs font-semibold">20</span>
      </button>

      <button @click="activeFilter = 'transaction'"
              :class="activeFilter === 'transaction' ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'"
              class="px-5 py-2.5 rounded-lg font-medium transition-all duration-200 flex items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg"
             fill="none"
             viewBox="0 0 24 24"
    ...
    {{-- Master Data Section --}}
    <div id="master-data-section">
      <div class="flex items-center gap-3 pt-4">
        <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-gradient-to-br from-purple-500 to-purple-600 shadow-lg">
          <svg xmlns="http://www.w3.org/2000/svg"
               fill="none"
               viewBox="0 0 24 24"
               stroke-width="1.5"
        ...
        <div>
          <h2 class="text-xl font-bold text-gray-900">Master Data</h2>
          <p class="text-sm text-gray-600">Base reference data and configurations</p>
        </div>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5 !mt-7">
        @php
          ...
          $transactionModules = [
              ...
              [
                  ...
                  'slug' => 'finished-good-slip',
                  'color' => 'green',
                  'description' => 'Finished Goods Item Slip',
                  'scope' => 'finished_good_slip_view',
              ...
              [
                  ...
                  'slug' => 'other-deposit',
                  'color' => 'blue',
                  'description' => 'Other deposit and cash receipts',
                  'scope' => 'other_deposit_view',
                  'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />',
              ],
              [
                  ...
                  'slug' => 'other-payment',
                  'color' => 'red',
                  'description' => 'Other payment and cash disbursements',
                  'scope' => 'other_payment_view',
                  'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />',
              ],
              [
              ...
              [
                  ...
                  'slug' => 'material-slip',
                  'color' => 'pink',
                  'description' => 'Material Item Retrieval Slip',
                  'scope' => 'material_slip_view',
                  'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />',
              ],
              [
        ...
        @foreach ($masterDataModules as $card)
          @php
          ...
          <div data-module-name="{{ $card['name'] }}"
               data-module-description="{{ $card['description'] }}"
               data-module-category="master"
            ...
            <div class="flex flex-col gap-5">
              <div class="flex items-center justify-between">
                <div class="flex items-center justify-center w-[60px] h-[60px] md:w-[70px] md:h-[70px] rounded-xl"
            ...
            <div class="flex flex-col gap-4">
              ...
              @if ($isCaptured)
                ...
                <button type="button"
                        @click.prevent="openCaptureModal('{{ $card['name'] }}', '{{ $card['slug'] }}', 'master')"
                        :disabled="capturing || switchingDb"
    ...
    <div id="transaction-section">
      <div class="flex items-center gap-3 pt-8">
        <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 shadow-lg">
          <svg xmlns="http://www.w3.org/2000/svg"
               fill="none"
               viewBox="0 0 24 24"
               stroke-width="1.5"
        ...
        <div>
          ...
          <p class="text-sm text-gray-600">Business transactions and operations</p>
        </div>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5 !mt-7">
        @foreach ($transactionModules as $card)
          @php
            $moduleData = $modules->firstWhere('slug', $card['slug']);
            $isCaptured = $moduleData !== null;
            $transactionCount = $isCaptured ? $moduleData->transactions_count : 0;
          ...
          <div data-module-name="{{ $card['name'] }}"
               data-module-description="{{ $card['description'] }}"
               data-module-category="transaction"
               class="rounded-xl border border-gray-200 min-h-[280px] md:min-h-[320px] shadow-xl bg-white flex flex-col justify-between flex-1 px-5 md:px-7 py-4 md:py-5">
            <div class="flex flex-col gap-5">
              <div class="flex items-center justify-between">
                <div class="flex items-center justify-center w-[60px] h-[60px] md:w-[70px] md:h-[70px] rounded-xl"
            ...
            <div class="flex flex-col gap-4">
              ...
              @if ($isCaptured)
                ...
                <button type="button"
                        @click.prevent="openCaptureModal('{{ $card['name'] }}', '{{ $card['slug'] }}', 'transaction')"
                        :disabled="capturing || switchingDb"
                        class="flex items-center justify-center gap-3 text-white font-semibold px-4 py-2 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed"
                        style="background: linear-gradient(to right, {{ $colors['from'] }}, {{ $colors['to'] }});"
                        onmouseover="this.style.background='linear-gradient(to right, {{ $colors['fromHover'] }}, {{ $colors['toHover'] }})'"
    ...
    <div class="w-full rounded-xl shadow-xl border-2 bg-blue-50 border-blue-200 flex flex-col sm:flex-row gap-3 p-4 md:p-5">
      ...
      <div x-show="showCaptureModal" class="fixed inset-0 z-[100] overflow-y-auto" style="display: none;" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
          <div x-show="showCaptureModal" x-transition.opacity class="fixed inset-0 transition-opacity bg-gray-900/60 backdrop-blur-sm" aria-hidden="true"></div>
          <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
          <div x-show="showCaptureModal" 
            ...
            <div class="px-6 py-5 bg-white border-b border-gray-100">
              <h3 class="text-xl font-bold text-gray-900" id="modal-title">Konfirmasi Capture Data</h3>
              <p class="mt-1 text-sm text-gray-500">Anda akan menarik data dari Accurate. Pastikan pengaturan di bawah ini sudah benar.</p>
            </div>
              ...
              <div class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col items-center text-center">
                    <span class="text-xs font-bold tracking-wider text-gray-500 uppercase mb-1">Source Database</span>
                    <span class="text-sm font-bold text-blue-600 mt-1">{{ $current_database_name ?? "Unknown Database" }}</span>
                  </div>
                  <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col items-center text-center">
                    <span class="text-xs font-bold tracking-wider text-gray-500 uppercase mb-1">Target Modul</span>
                    <span class="text-sm font-bold text-indigo-600 mt-1" x-text="modalModuleName"></span>
                  </div>
                </div>

                <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                  <span class="block text-xs font-bold tracking-wider text-gray-500 uppercase mb-2">Filter Tanggal (Opsional)</span>
                  <div class="grid grid-cols-2 gap-3">
                    <div>
                      <label class="block text-xs text-gray-600 mb-1">Start Date</label>
                      <input type="date" x-model="modalStartDate" class="w-full border-gray-300 rounded-lg text-sm bg-gray-50 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
                      <label class="block text-xs text-gray-600 mb-1">End Date</label>
                      <input type="date" x-model="modalEndDate" class="w-full border-gray-300 rounded-lg text-sm bg-gray-50 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                  </div>
                  
                  <div class="mt-3 pt-3 border-t border-gray-100">
                    <label class="block text-xs text-gray-600 mb-2">Tipe Filter</label>
                    <div class="grid grid-cols-2 gap-2 text-xs">
                      <label class="inline-flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1.5 rounded">
                        <input type="radio" x-model="modalFilterType" name="modal_filter_type" value="range" class="text-blue-600 border-gray-300 focus:ring-blue-500">
                        <span class="font-medium text-gray-700">Range Trans Date</span>
                      </label>
                      <label class="inline-flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1.5 rounded">
                        <input type="radio" x-model="modalFilterType" name="modal_filter_type" value="equal" class="text-blue-600 border-gray-300 focus:ring-blue-500">
                        <span class="font-medium text-gray-700">Equal Trans Date</span>
                      </label>
                      <label class="inline-flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1.5 rounded">
                        <input type="radio" x-model="modalFilterType" name="modal_filter_type" value="last_update" class="text-blue-600 border-gray-300 focus:ring-blue-500">
                        <span class="font-medium text-gray-700">Range Last Update</span>
                      </label>
                      <label class="inline-flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1.5 rounded">
                        <input type="radio" x-model="modalFilterType" name="modal_filter_type" value="last_update_equal" class="text-blue-600 border-gray-300 focus:ring-blue-500">
                        <span class="font-medium text-gray-700">Equal Last Update</span>
                      </label>
                    </div>
                  </div>
                </div>
                ...
                <div class="bg-blue-50/50 p-4 rounded-xl border border-blue-100 flex items-start gap-3">
                  ...
                  <div>
                    <p class="text-sm font-semibold text-blue-800">Mode Capture: <span x-text="modalCaptureMode.replaceAll('_', ' ')" class="uppercase"></span></p>
                    <p class="text-xs text-blue-600 mt-1">Data akan ditarik di background. Anda dapat meninggalkan halaman ini selagi proses berjalan.</p>
                  </div>
                </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 sm:flex sm:flex-row-reverse gap-3">
              <button @click="executeCapture()" type="button" class="inline-flex justify-center w-full px-6 py-2.5 text-sm font-bold text-white bg-blue-600 border border-transparent rounded-xl shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:w-auto transition-all active:scale-95">Mulai Capture</button>
              <button @click="closeCaptureModal()" type="button" class="mt-3 sm:mt-0 inline-flex justify-center w-full px-6 py-2.5 text-sm font-bold text-gray-700 bg-white border border-gray-300 rounded-xl shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-200 sm:w-auto transition-all active:scale-95">Batal</button>
            </div>
          </div>
        </div>
      </div>
