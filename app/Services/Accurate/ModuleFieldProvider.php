<?php

namespace App\Services\Accurate;

class ModuleFieldProvider
{
    private array $moduleFieldMap = [
        'vendor' => 'vendorNo',
        'customer' => 'customerNo',
        'item' => 'no',
        'glaccount' => 'no',
        'employee' => 'no',
        'tax' => 'no',
        'project' => 'no',
        'warehouse' => 'id',
        'branch' => 'id',
        'department' => 'id',
        'sales-order' => 'number',
        'purchase-order' => 'number',
        'sales-invoice' => 'charField1',
        'down-payment-sales-invoice' => 'charField1',
        'purchase-invoice' => 'charField1',
        'down-payment-purchase-invoice' => 'charField1',
        'delivery-order' => 'number',
        'receive-item' => 'charField1',
        'sales-quotation' => 'number',
        'purchase-requisition' => 'number',
        'sales-return' => 'number',
        'purchase-return' => 'number',
        'sales-receipt' => 'charField1',
        'purchase-payment' => 'charField1',
        'item-transfer' => 'number',
        'item-adjustment' => 'number',
        'job-order' => 'number',
        'work-order' => 'number',
        'item-category' => 'id',
        'unit' => 'id',
        'vendor-category' => 'id',
        'vendor-claim' => 'id',
        'vendor-price' => 'id',
        'customer-category' => 'id',
        'currency' => 'id',
        'fob' => 'id',
        'data-classification' => 'id',
        'price-category' => 'id',
        'bill-of-material' => 'id',
    ];

    public function getNumberFieldForModule(string $module, array $firstItem): ?string
    {
        if (isset($this->moduleFieldMap[$module])) {
            return $this->moduleFieldMap[$module];
        }

        $possibleFields = ['no', 'vendorNo', 'customerNo', 'number'];

        foreach ($possibleFields as $field) {
            if (isset($firstItem[$field]) && !empty($firstItem[$field])) {
                return $field;
            }
        }

        return null;
    }
}
